...
router.post('/submit', function(req, res) {
  ...
  files.forEach(element => {
    temp.push({id: idtp, name: element, status: 'todo'});
    idtp+=1;
  });
  tableau[idrequest]=({idrequest: idrequest, files:temp});
...
router.post('/update', async function(req, res) {
  ...
  while (files.length > 0 &&
    ...
    files.sort().every(function(value, index) { return _.isEqual(value, tableau[idrequest].files.sort()[index])}) &&
    files.sort().some(function(value) { return value.status !== 'ended'})
    ) {
      //console.log(req.body);
      //console.log(tableau);
      await sleep(500);
      ...
      if (element.status != 'running') {
        ...
        if (element.status == 'todo'){
          ...
          fs.rename('/tmp/tp/running/'+element.name, '/tmp/tp/done/'+element.name, (err) => {
            ...
            element.result = [{id:1254, state:'success'},{id:12554, state:'success'}];
          })
        }
      }
    });
